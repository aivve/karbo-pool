...
function recordShareData(miner, job, shareDiff, blockCandidate, hashHex, shareType, blockTemplate){
    ...
    var redisCommands = [
        ...
        ['zadd', config.coin + ':hashrate', dateNowSeconds, [job.difficulty, miner.login, dateNow, miner.ip, miner.pass].join(':')],
        ...
        ['hincrby', config.coin + ':workersByIp:' + miner.login + ':' + miner.ip, 'hashes', job.difficulty],
        ['hmset', config.coin + ':workersByIp:' + miner.login + ':' + miner.ip, 'lastShare', dateNowSeconds, 'difficulty', miner.difficulty],
        ['hincrby', config.coin + ':workersByIp:' + miner.login + ':' + miner.ip, 'accepted', 1],
        ['expire', config.coin + ':workersByIp:' + miner.login + ':' + miner.ip, 900],
        ['hincrby', config.coin + ':workersByN:' + miner.login + ':' + miner.pass, 'hashes', job.difficulty],
        ['hmset', config.coin + ':workersByN:' + miner.login + ':' + miner.pass, 'lastShare', dateNowSeconds, 'difficulty', miner.difficulty],
        ['hincrby', config.coin + ':workersByN:' + miner.login + ':' + miner.pass, 'accepted', 1],
        ['expire', config.coin + ':workersByN:' + miner.login + ':' + miner.pass, 900]
    ];
...
function recordBadShareData(miner, job, hashHex, shareType) {

    var dateNow = Date.now();
    var dateNowSeconds = dateNow / 1000 | 0;

    var redisCommands = [
        ['hincrby', config.coin + ':workersByIp:' + miner.login + ':' + miner.ip, shareType, 1],
        ['expire', config.coin + ':workersByIp:' + miner.login + ':' + miner.ip, 900],
        ['hincrby', config.coin + ':workersByN:' + miner.login + ':' + miner.pass, shareType, 1],
        ['expire', config.coin + ':workersByN:' + miner.login + ':' + miner.pass, 900]
    ];

    redisClient.multi(redisCommands).exec(function(err, replies) {
        if (err) {
            log('error', logSystem, 'Failed to insert info about bad share data into redis %j \n %j', [err, redisCommands]);
            return;
        }
    });

    log('info', logSystem, 'Bad %s share at difficulty %d from %s@%s', [shareType, job.difficulty, miner.login, miner.ip]);
}
...
function processShare(miner, job, blockTemplate, nonce, resultHash){
	...
	recordBadShareData(miner, job, hash, 'expired');
        return false;
    }
	...
	recordBadShareData(miner, job, hash, 'bad');
        return false;
    }
    else{
        recordShareData(miner, job, hashDiff.toString(), false, null, shareType);
...
function handleMinerMethod(method, params, ip, portData, sendReply, pushMessage){
    ...
    switch(method){
        case 'login':
            ...
            var rigName = params.pass.trim().substring(0, 32).replace(/[^a-zA-Z0-9_-]/g, '_');
            miner = new Miner(minerId, login, rigName, ip, difficulty, noRetarget, pushMessage);
            ...
            log('info', logSystem, 'Miner connected %s@%s RIG: %s',  [params.login, miner.ip, rigName]);
            break;
        case 'getjob':
            if (!miner){
